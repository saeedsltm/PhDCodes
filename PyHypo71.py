...
def checkUserInfo():
    if len(sys.argv) != 2:
        print("+++ Wrong inputs, correct syntax is 'PyHypo71.py rootName'")
        sys.exit()
...
def runHypo71(rootName):
    ...
    if platform.system() == "Linux":
        hypo71 = shutil.which("hypo71")
        cmd = "{0} < {1}.inp > /dev/null".format(hypo71, rootName)
    elif platform.system() == "Windows":
        cmd = "utils/hypo71Main.exe < {0}.inp > NUL".format(rootName)
    createHypo71PhaseFile(rootName)
    createHypo71InputFile(rootName)
    os.system(cmd)
    RMS, ERH, ERZ = getStatistic(rootName)
    for i in ["{0}_h71.res".format(rootName),
