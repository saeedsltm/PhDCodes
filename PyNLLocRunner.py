...
import os, sys
from numpy import array, arange, arccos, char, pi, sqrt, linalg ,arctan2
...
class main():
    ...
    def extract_nlloc_hyp(self, root_name):
        ...
        with open(out_evt.name, 'a') as f, open(out_pha.name, 'a') as g:
            ...
            hdr_f = '#    OT                     LON      LAT      DEP      RMS      GAP      ERH      ERZ      CXX      CXY      CXZ      CYY      CYZ      CZZ   USD_ST   USD_PH   MIN_DS   MAX_DS   AVG_DS'
            ...
            for loc_file in loc_files:
                ...
                with open(loc_file) as inp:
                    ...
                    for line in inp:
                        ...
                        if 'GEOGRAPHIC' in l:
                            ...
                            evt_res = append(evt_res, dt.strptime(line[:42].strip(),'GEOGRAPHIC  OT %Y %m %d  %H %M %S.%f')) 
                        ...
                        if evt_res.size and 'STAT_GEOG' in l and int(evt_res[2]) == 0:
                ...
                savetxt(f, evt_res, newline=' ', fmt='%s')
...
#        plt.rc('text', usetex=True)
        ...
        c1 = char.startswith(pha_data[:,1],'P')
        ...
        sc = ax.scatter(x, y, s=5, c=z, lw=0.1, vmax=self.max_dist, cmap=plt.cm.seismic)
        cb = plt.colorbar(sc)
        ...
        ax.grid(True, linestyle=':', linewidth=.5, color='k', alpha=.3)
        c1 = char.startswith(pha_data[:,1],'S')
        c2 = array(pha_data[:,5], dtype=float)<=self.max_dist
        x  = array(pha_data[(c1)&(c2)][:,3], dtype=float)
        y  = array(pha_data[(c1)&(c2)][:,4], dtype=float)
        z  = array(pha_data[(c1)&(c2)][:,5], dtype=float)
        ...
        ax.grid(True, linestyle=':', linewidth=.5, color='k', alpha=.3)
        x  = array(pha_data[char.startswith(pha_data[:,1],'P')][:,5], dtype=float)
        y  = array(pha_data[char.startswith(pha_data[:,1],'P')][:,3], dtype=float)
        sc = ax.scatter(x, y, s=5, c='r', lw=0.2, alpha=.6, label='P')
        x  = array(pha_data[char.startswith(pha_data[:,1],'S')][:,5], dtype=float)
        y  = array(pha_data[char.startswith(pha_data[:,1],'S')][:,3], dtype=float)
        ax.set_xlim(0, self.max_dist)
        ax.set_ylim(-self.s_res_max, self.s_res_max)
        sc = ax.scatter(x, y, s=5, c='b', lw=0.2, alpha=.6, label='S')
        ...
        h = -evt_data[evt_data[:,4]<=self.dep_max][:,4]
        w = self.dep_hist_bw
        b = arange(min(h), max(h) + w, w)
        a,_,_ = ax.hist(h, b, orientation='horizontal', rwidth=0.8, edgecolor='r', facecolor='gray')
        ax.set_xlabel('No. of event')
        ax.set_ylabel('Depth (km)')
        ax.set_title('Total No. of events=%d'%sum(a))
        ax.grid(True, linestyle=':', linewidth=.5, color='k', alpha=.3)
        ax.locator_params(axis = 'x', nbins=5)
        ax.locator_params(axis = 'y', nbins=5)       
        ...
        h = evt_data[evt_data[:,7]<=self.Herr_max][:,7]
        w = self.her_hist_bw
        b = arange(min(h), max(h) + w, w)
        a,_,_ = ax.hist(h, b, rwidth=0.8, edgecolor='r', facecolor='gray')
        ax.set_xlabel('Horizontal Error (km)')
        ax.set_ylabel('No. of event')
        ax.set_title('Total No. of events=%d'%sum(a))
        ax.grid(True, linestyle=':', linewidth=.5, color='k', alpha=.3)
        ax.locator_params(axis = 'x', nbins=5)
        ax.locator_params(axis = 'y', nbins=5)
        ...
        h = evt_data[evt_data[:,8]<=self.Zerr_max][:,8]
        w = self.zer_hist_bw
        b = arange(min(h), max(h) + w, w)
        a,_,_ = ax.hist(h, b, rwidth=0.8, edgecolor='r', facecolor='gray')
        ...
        h = evt_data[evt_data[:,5]<=self.rms_max][:,5]
        w = self.rms_hist_bw
        b = arange(min(h), max(h) + w, w)
        a,_,_ = ax.hist(h, b, rwidth=0.8, edgecolor='r', facecolor='gray')
        ...
        x = evt_data[:,4][evt_data[:,17]<=self.minds_max]
        y = evt_data[:,17][evt_data[:,17]<=self.minds_max]
        z = evt_data[:,6][evt_data[:,17]<=self.minds_max]
        sc = ax.scatter(x, y, s=8, c=z, lw=0.2, alpha=.6, label='P', cmap=plt.cm.seismic)
        cb = plt.colorbar(sc)
        cb.set_label('Az Gap (deg)')
        tick_locator = ticker.MaxNLocator(nbins=6)
        cb.locator = tick_locator
        cb.update_ticks()
        ax.hlines(y=20, xmin=0, xmax=20, linestyles=':')
        ax.vlines(x=20, ymin=0, ymax=20, linestyles=':')
        ax.set_xlabel('Depth (km)')
        ax.set_ylabel('Min Distance (km)')
        ax.set_xlim(0, self.dep_max)
        ax.set_ylim(0, self.minds_max)
        ax.grid(True, linestyle=':', linewidth=.5, color='k', alpha=.3)
        ax.set_title('Total No. of events=%d'%x.size)
        ax.locator_params(axis = 'x', nbins=5)
        ax.locator_params(axis = 'y', nbins=5)
        ...
        x = evt_data[:,6][evt_data[:,7]<=self.Herr_max]
        y = evt_data[:,7][evt_data[:,7]<=self.Herr_max]
        z = evt_data[:,15][evt_data[:,7]<=self.Herr_max]
        sc = ax.scatter(x, y, s=8, c=z, lw=0.2, alpha=.6, label='S', cmap=plt.cm.seismic)
        cb = plt.colorbar(sc)
        cb.set_label('No. of used station (deg)')
        tick_locator = ticker.MaxNLocator(nbins=6)
        cb.locator = tick_locator
        cb.update_ticks()
        ax.set_xlabel('Az Gap (deg)')
        ax.set_ylabel('Hr Error (km)')
        ax.set_xlim(0, 360)
...
#        plt.rc('text', usetex=True)
        ...
        c1  = evt_data[:,7]<=self.Herr_max
        c2  = evt_data[:,8]<=self.Zerr_max
        
        cxx = evt_data[(c1)&(c2)][:,9]
        cxy = evt_data[(c1)&(c2)][:,10]
        cyy = evt_data[(c1)&(c2)][:,12]
        lon = evt_data[(c1)&(c2)][:,2]
        lat = evt_data[(c1)&(c2)][:,3]
        ...
        for x, y, xx, xy, xy, yy in zip(lon, lat, cxx, cxy, cxy, cyy):
            ...
            ax.add_artist(ell)
            ax.scatter(x, y, color='r', marker='*', zorder=100)
    ...
    #_________ CONVERT NLLOC OUTPUT TO NORDIC

    def hyp2nordic(self, root_name):

        here = os.getcwd()
        os.mkdir('tmp') 
        hyp_files = glob(os.path.join('loc','*%s*.hyp'%root_name))
        
        for hyp_file in hyp_files:

            copy(hyp_file, os.path.join('tmp'))

        os.chdir(os.path.join('tmp'))
        cmd = "for i in `ls *`; do cat $i >> nordic.dat;done"
        os.system(cmd)
        
        with open('nlloctools.par','w') as f:
        
            f.write('2\n2\nnordic.dat\n')

        cmd = 'PyNLLocTools.py < nlloctools.par > /dev/null'
        os.system(cmd)

        copy('nlloc.out', os.path.join('..'))
        os.chdir(here)
        cmd = 'rm -rf tmp'
        os.system(cmd)
...
start.hyp2nordic(root_name=start.nlloc_dic['LOCFILES_OUTPUT'])
